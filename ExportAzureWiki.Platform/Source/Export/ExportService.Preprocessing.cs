...
namespace ExportAzureWiki
{
    public partial class ExportService
    {
        ...
        {
            htmlContent = htmlContent.Replace("@null", "0", StringComparison.OrdinalIgnoreCase);

            var doc = new HtmlDocument();
            doc.LoadHtml(htmlContent);
            NormalizeMermaidForPdf(doc);
            EnsurePdfExportStyles(doc);

            // Workaround for iText SVG marker bug (InvalidCastException on marker-capable cast).
            ...
            if (svgNodes != null)
            {
                foreach (var svg in svgNodes)
                {
                    ...
                    var markerNodes = svg.SelectNodes(".//*[local-name()='marker' or name()='marker']");
                    if (markerNodes != null)
                    {
                        foreach (var marker in markerNodes.ToList())
                        {
                            marker.Remove();
                        }
                    }

                    var withMarkerAttrs = svg.SelectNodes(".//*[@marker-start or @marker-mid or @marker-end]");
                    if (withMarkerAttrs != null)
                    {
                        foreach (var node in withMarkerAttrs)
                        {
                            node.Attributes.Remove("marker-start");
                            node.Attributes.Remove("marker-mid");
                            node.Attributes.Remove("marker-end");
                        }
                    }
                }
            }
        ...
        private static void NormalizeMermaidForPdf(HtmlDocument doc)
        {
            // Normalize raw Mermaid blocks to deterministic image rendering for PDF.
            var mermaidDivs = doc.DocumentNode.SelectNodes("//div[contains(concat(' ', normalize-space(@class), ' '), ' mermaid ')]");
            if (mermaidDivs != null)
            {
                foreach (var mermaidDiv in mermaidDivs.ToList())
                {
                    ...
                    var code = doc.CreateElement("code");
                    code.SetAttributeValue("class", "language-mermaid");
                    code.InnerHtml = WebUtility.HtmlEncode(WebUtility.HtmlDecode(mermaidDiv.InnerText ?? string.Empty).Trim());
                    var pre = doc.CreateElement("pre");
                    pre.AppendChild(code);
                    mermaidDiv.ParentNode.ReplaceChild(pre, mermaidDiv);
                }
            ...
            var mermaidCodeBlocks = doc.DocumentNode.SelectNodes("//pre[.//code[contains(@class,'language-mermaid') or contains(@class,'mermaid')]]");
            if (mermaidCodeBlocks != null)
            {
                foreach (var pre in mermaidCodeBlocks.ToList())
                {
                    var source = WebUtility.HtmlDecode(pre.InnerText ?? string.Empty).Trim();
                    if (string.IsNullOrWhiteSpace(source))
                    {
                        continue;
                    }

                    var normalizedSource = SanitizeMermaidSourceForPdf(source);
                    var imageUrl = TryBuildMermaidInkUrl(normalizedSource);
                    if (string.IsNullOrWhiteSpace(imageUrl))
                    {
                        continue;
                    }
                    ...
                    img.SetAttributeValue(
                        "style",
                        "display:block;max-width:100%;width:auto;height:auto;max-height:520px;object-fit:contain;margin:10px auto;");
                    pre.ParentNode.ReplaceChild(img, pre);
                }
            }

            // Clamp already-rendered Mermaid canvases/SVGs to avoid giant blank canvas in PDF.
            var mermaidVisuals = doc.DocumentNode.SelectNodes("//div[contains(concat(' ', normalize-space(@class), ' '), ' mermaid ')]//svg | //div[contains(concat(' ', normalize-space(@class), ' '), ' mermaid ')]//canvas");
            if (mermaidVisuals != null)
            {
                foreach (var visual in mermaidVisuals)
                {
                    visual.Attributes.Remove("width");
                    visual.Attributes.Remove("height");
                    visual.SetAttributeValue("style", "max-width:100%;width:100%;height:auto;");
                }
            }
        }

        private static string SanitizeMermaidSourceForPdf(string source)
        {
            if (string.IsNullOrWhiteSpace(source))
            {
                return source;
            ...
            // Keep PDF readable when source forces white text in nodes.
            source = Regex.Replace(source, @"(?i)color\s*:\s*#(?:fff|ffffff)\b", "color:#111111");
            return source;
        }

        private static void EnsurePdfExportStyles(HtmlDocument doc)
        {
            ...
            var existing = head.SelectSingleNode(".//style[@id='export-pdf-quality-style']");
            if (existing != null)
            {
                return;
            }
            ...
            styleNode.SetAttributeValue("id", "export-pdf-quality-style");
            styleNode.InnerHtml = @"
@page {
  size: A4;
...
pre code span { opacity: 1 !important; }
.hljs-comment, .hljs-quote { color: #6b7280 !important; }
.hljs-keyword, .hljs-selector-tag, .hljs-literal { color: #0f766e !important; font-weight: 600 !important; }
.hljs-string, .hljs-doctag, .hljs-regexp { color: #b45309 !important; }
.hljs-number, .hljs-symbol, .hljs-bullet { color: #1d4ed8 !important; }
.hljs-title, .hljs-title.function_, .hljs-function .hljs-title { color: #7c3aed !important; }
.hljs-built_in, .hljs-type, .hljs-class .hljs-title { color: #0e7490 !important; }
.hljs-params { color: #334155 !important; }
table {
  width: 100% !important;
...
pre, blockquote, table, img, svg, figure, .mermaid, .codehilite, .highlight, .panel, .alert, .admonition {
  page-break-inside: avoid !important;
  break-inside: avoid-page !important;
}
pre, blockquote, .mermaid, .codehilite, .highlight, .panel, .alert, .admonition {
...
";
        ...
        /// <summary>
        /// The on-disk image cache is encrypted at rest, but the Word/PDF
        /// embedders (OpenXML FileStream, iText file:// loader, SVG/WebP
        /// transcode) read raw files from disk. This decrypts every image
        ...
        private static void EnsureNumericAttr(HtmlNode node, string attrName, string fallback)
        {
            ...
            // iText requires numeric absolute values for critical SVG dimensions.
